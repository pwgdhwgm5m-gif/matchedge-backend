...
test('coverage metadata separates mapping, fixture, result, filter and home states', () => {
  ...
  for (const [provider, leagueId] of [
    ['sportsdb', '4631'],
    ['sportsdb', '5193'],
    ['sportsdb', '5635']
  ]) {
    const mapped = registry.resolveCompetition({ provider, leagueId });
    assert.equal(mapped.mappingStatus, 'verified');
    assert.equal(mapped.fixtureCoverage, 'fixture-producing');
  ...
  for (const key of ['concacaf-nations-league']) {
  ...
  const mexico = registry.getCompetitionRegistry().find(item => item.canonicalCompetitionKey === 'mexico-liga-mx');
  assert.equal(mexico.visibleInCompetitionFilter, true);
  assert.equal(mexico.eligibleForHomePriority, false);
