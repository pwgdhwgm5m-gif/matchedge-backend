...
async function rebuildAllAnalystStats(){await User.updateMany({},{$set:{correctPicks:0,wrongPicks:0,currentStreak:0,bestStreak:0}});const ids=await CommunityPick.distinct('userId',{result:{$in:['won','lost']}});for(const id of ids)await rebuildUserStats(id);return ids.length}
