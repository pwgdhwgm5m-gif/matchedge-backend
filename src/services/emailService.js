...
async function sendVerificationEmail(to, { token, code }) {
  ...
  return sendEmail({
    to,
    subject: 'MatchEdge doğrulama kodun',
    html: `
      <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:480px;margin:0 auto;background:#0a0e17;color:#f2f4f8;padding:28px;border-radius:18px;">
        <div style="font-size:20px;font-weight:800;margin-bottom:6px;">MATCHEDGE <span style="color:#2dd4bf;">PREMIUM</span></div>
        <p style="color:#a8b0c2;line-height:1.5;">Hesabını doğrulamak için aşağıdaki 6 haneli kodu MatchEdge ekranına yaz:</p>
...
async function sendPasswordResetEmail(to, token) {
  ...
  return sendEmail({
    to,
    subject: 'MatchEdge - Şifre sıfırlama',
    html: `
