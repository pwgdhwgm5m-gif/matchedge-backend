...
router.get('/:fixtureId/v4-intelligence',async(req,res)=>{try{const fixtureId=String(req.params.fixtureId);const [sim,similar,patterns]=await Promise.all([premiumLab.simulateFixture(fixtureId).catch(e=>({unavailable:true,error:e.message})),premiumLab.similarMatches(fixtureId,{limit:20}).catch(e=>({unavailable:true,error:e.message})),premiumLab.patternFinder({}).catch(e=>({unavailable:true,error:e.message}))]);let value={unavailable:true};try{const vf=await premiumLab.valueFinder({limit:80});const m=(vf.matches||[]).find(x=>String(x.fixtureId)===fixtureId);value=m||{unavailable:true,providerAvailable:vf.providerAvailable,note:vf.note}}catch(e){value={unavailable:true,error:e.message}}const row=await ledger.reportCard(fixtureId);const lockedV4=await premiumLab.fixtureValidation(fixtureId);const pick=row.strongestPick||null,mc=sim.simulation&&pick?premiumLab.monteCarlo50k({fixtureId,homeLambda:sim.simulation.expectedGoals?.home,awayLambda:sim.simulation.expectedGoals?.away}):null;let simProb=null;if(mc&&pick){const k=pick.key;simProb=k==='home'?mc.home:k==='draw'?mc.draw:k==='away'?mc.away:k==='over25'?mc.over25:k==='under25'?mc.under25:k==='bttsYes'?mc.bttsYes:k==='bttsNo'?mc.bttsNo:null}const modelProb=Number(pick?.probability);const gap=Number.isFinite(modelProb)&&simProb!=null?+Math.abs(modelProb-simProb).toFixed(1):null;const agreement=lockedV4?.agreement||(gap==null?'unavailable':gap<=4?'strong':gap<=8?'aligned':gap<=15?'mixed':'conflict');const health=Number((await require('../models/PredictionSnapshot').findOne({fixtureId}).select('dataQualityScore').lean())?.dataQualityScore||0);const strongEdge=!!pick&&modelProb>=60&&health>=55&&(agreement==='strong'||agreement==='aligned');const edgeClass=!pick?'NO_EDGE':strongEdge?'STRONG_EDGE':agreement==='conflict'||health<45?'RISKY':'WATCH';res.json({engine:'premium-v4-intelligence',fixtureId,edgeId:row.edgeId,strongestPick:pick,edgeDecision:{classification:edgeClass,strongEdge,modelProbability:modelProb,dataQualityScore:health,v4Agreement:agreement,rule:'AI >=60 + data health >=55 + V4 strong/aligned'},lockedValidation:lockedV4,simulation:mc?{runs:mc.runs,probability:simProb,gap,agreement:gap==null?'unavailable':gap<=4?'strong':gap<=8?'aligned':gap<=15?'mixed':'conflict',expectedGoals:sim.simulation.expectedGoals,mostLikelyScores:sim.simulation.mostLikelyScores}:sim,similarMatches:similar,patterns,value,policy:{primaryModel:'socceredge-ai',v4Role:'cross-check-and-evidence',weakFillerAllowed:false,noValueClaimWithoutVerifiedOdds:true}})}catch(e){res.status(e.status||500).json({error:'v4_intelligence_unavailable',detail:e.message})}});
router.post('/v4-decisions',async(req,res)=>{try{const ids=[...new Set((req.body?.fixtureIds||[]).map(String))].slice(0,60);const Prediction=require('../models/PredictionSnapshot');const rows=await Prediction.find({fixtureId:{$in:ids},status:'pending'}).select('fixtureId strongestPick dataQualityScore v4Validation').lean();res.json({items:rows.map(x=>{const v=(x.v4Validation||[]).find(z=>z?.kind==='fixture-cross-check'),p=Number(x.strongestPick?.probability),health=Number(x.dataQualityScore||0),agreement=v?.agreement||'unavailable',strongEdge=!!x.strongestPick&&p>=60&&health>=55&&(agreement==='strong'||agreement==='aligned');return{fixtureId:x.fixtureId,strongestPick:x.strongestPick,classification:!x.strongestPick?'NO_EDGE':strongEdge?'STRONG_EDGE':agreement==='conflict'||health<45?'RISKY':'WATCH',strongEdge,dataQualityScore:health,v4Agreement:agreement}})});}catch(e){res.status(500).json({error:'v4_decisions_unavailable'})}});
router.get('/model/v4-performance',async(req,res)=>{try{res.json({rows:await ledger.v4CrossCheckPerformance()})}catch(e){res.status(500).json({error:'v4_performance_unavailable'})}});
